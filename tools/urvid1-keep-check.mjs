...
for (let s = 1; s <= 40; s++) {
  const base = makeVideo({ ...brief, seed: s, perSceneAnims: true })
  const rr = makeVideo({ ...brief, seed: (s + 0x9e3779b1) >>> 0, perSceneAnims: true, keepRecipe: base.recipe })
  for (const k of OPT) {
...
for (let s = 1; s <= 40; s++) {
  ...
  const rr = makeVideo({ ...brief, seed: s, perSceneAnims: true, keepRecipe: keepEmpty })
...
for (let s = 1; s <= 60 && !everPresent; s++) {
  ...
  everPresent = true
  const rr = makeVideo({ ...brief, seed: (s * 7 + 1) >>> 0, perSceneAnims: true, keepRecipe: { color: 'x', type: 'x', post: base.recipe.post, typekit: null, mark: null, sub: null, atm: null } })

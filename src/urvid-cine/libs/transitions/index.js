...
// --- cut + fade-cut ---
T('transitions.cut.hard', (ctx, p, drawA, drawB) => { if (p < 0.5) drawA(ctx); else drawB(ctx) }, { weight: 1.2, tags: ['corte', 'seco'] })

// --- wipes (B se revela por una mascara que barre; A queda debajo, oculta donde entra B) ---
T('transitions.wipe.right', (ctx, p, drawA, drawB) => { const e = eOutCubic(p); drawA(ctx); clipRect(ctx, 0, 0, W * e, H, drawB) }, { weight: 1.1, tags: ['wipe', 'barrido'] })
T('transitions.wipe.left', (ctx, p, drawA, drawB) => { const e = eOutCubic(p); drawA(ctx); clipRect(ctx, W * (1 - e), 0, W * e, H, drawB) }, { weight: 1, tags: ['wipe', 'barrido'] })
T('transitions.wipe.up', (ctx, p, drawA, drawB) => { const e = eOutCubic(p); drawA(ctx); clipRect(ctx, 0, H * (1 - e), W, H * e, drawB) }, { weight: 1, tags: ['wipe', 'barrido'] })
T('transitions.wipe.down', (ctx, p, drawA, drawB) => { const e = eOutCubic(p); drawA(ctx); clipRect(ctx, 0, 0, W, H * e, drawB) }, { weight: 1, tags: ['wipe', 'barrido'] })

// --- push (A sale empujada por B; viajan pegadas, sin solapamiento) ---
T('transitions.push.left', (ctx, p, drawA, drawB) => { const e = eInOutCubic(p); ctx.save(); ctx.translate(-W * e, 0); drawA(ctx); ctx.restore(); ctx.save(); ctx.translate(W * (1 - e), 0); drawB(ctx); ctx.restore() }, { weight: 1, tags: ['push', 'lateral'] })
T('transitions.push.right', (ctx, p, drawA, drawB) => { const e = eInOutCubic(p); ctx.save(); ctx.translate(W * e, 0); drawA(ctx); ctx.restore(); ctx.save(); ctx.translate(-W * (1 - e), 0); drawB(ctx); ctx.restore() }, { weight: 0.9, tags: ['push', 'lateral'] })
T('transitions.push.up', (ctx, p, drawA, drawB) => { const e = eInOutCubic(p); ctx.save(); ctx.translate(0, -H * e); drawA(ctx); ctx.restore(); ctx.save(); ctx.translate(0, H * (1 - e)); drawB(ctx); ctx.restore() }, { weight: 0.9, tags: ['push', 'vertical'] })

// --- slide-over (B entra deslizando ENCIMA de A desde un borde) ---
T('transitions.slide.over-right', (ctx, p, drawA, drawB) => { const e = eOutCubic(p); drawA(ctx); ctx.save(); ctx.translate(W * (1 - e), 0); drawB(ctx); ctx.restore() }, { weight: 0.95, tags: ['slide', 'over'] })
T('transitions.slide.over-up', (ctx, p, drawA, drawB) => { const e = eOutCubic(p); drawA(ctx); ctx.save(); ctx.translate(0, H * (1 - e)); drawB(ctx); ctx.restore() }, { weight: 0.9, tags: ['slide', 'over'] })
...
}, { weight: 0.75, tags: ['blinds', 'persianas'] })
...
}, { weight: 0.7, tags: ['wedge', 'reloj', 'radial'] })
...
}, { weight: 0.85, tags: ['wipe', 'diagonal'] })

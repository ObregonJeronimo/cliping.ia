// urvid 1.0 · PRNG determinista. mulberry32 + sub-seeds por NAMESPACE (modelo fxhash/SFC32): iterar un generador
// (ej 'color') NO desbaraja otro (ej 'motion'). REGLA DURA: cero Math.random / Date.now en todo el motor.
...
// stable_seed: marca + rubro -> semilla estable (mismo input -> mismo video, siempre)
export function stableSeed(...parts) { return hashStr(parts.filter(p => p != null).join('|')) }

// seedFor: un generador con SUB-SEED por namespace -> ejes ortogonales (color/motion/layout...) independientes
export function seedFor(seed, namespace) { return mulberry32((seed ^ hashStr(namespace)) >>> 0) }

// helpers de muestreo sobre un prng (función que devuelve [0,1))
...
export function weightedPick(prng, items, weightOf) {   // distribución sesgada (QQL/Hobbs)

    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.cols}>
        <div className={styles.panel}>
          ...
          <div className={styles.field}>Tono<div className={styles.seg}>{['dark', 'light'].map(tn => <button key={tn} className={brief.tone === tn ? styles.on : ''} onClick={() => setTone(tn)}>{tn === 'dark' ? 'oscuro' : 'claro'}</button>)}</div></div>
          <div className={styles.field}>Formato<div className={styles.seg}>{[['9:16', 'Reel'], ['4:5', 'Feed'], ['1:1', 'Cuadr.']].map(([f, lbl]) => <button key={f} className={(brief.format || '9:16') === f ? styles.on : ''} onClick={() => up('format', f)}>{lbl} {f}</button>)}</div></div>
          <div className={styles.field}>Duración<div className={styles.seg}>{[['corto', 'Corto'], ['medio', 'Medio'], ['largo', 'Largo']].map(([d, lbl]) => <button key={d} className={(brief.duration || 'medio') === d ? styles.on : ''} onClick={() => up('duration', d)}>{lbl}</button>)}</div></div>
          <div className={styles.field}>Dirección<div className={styles.seg}>{[['', 'Auto'], ['premium', 'Premium ✦']].map(([d, lbl]) => <button key={lbl} className={(brief.style || '') === d ? styles.on : ''} onClick={() => up('style', d || undefined)}>{lbl}</button>)}</div></div>
          <div className={styles.field}>Público<div className={styles.seg}>{SEGMENTS.map(s => <button key={s.key} className={segment === s.key ? styles.on : ''} onClick={() => applySegment(s)} title={s.key === 'auto' ? 'Usa el público inferido por la IA o los defaults del rubro' : `${s.register} · ${s.awareness} · seriedad ${s.seriousness}`}>{s.label}</button>)}</div></div>

...
> Leer ESTO primero al retomar. Estado al commit `6e77c94` (jun 2026; 567 modulos: Ola 4 color + motion + typekit + markkit/datakit + transiciones + cerebro v2 + Ola 5). Complementa: `docs/URVID-1.0-BLUEPRINT.md`
...
- **libs/** (`index.js` importa todas) — 11 bibliotecas:
  ...
  | backgrounds | `backgrounds` | 64 |
  | substrates | `substrates` | 67 |
  | atmosphere | `atmosphere` | 60 |
  | scenes | **`scene-layouts`** | 58 |
  | markkit | `markkit` | 68 |
  | datakit | `datakit` | 52 |
...
2. **[HECHO]** **transitions/** (16, sin buffers: wipes/slides/iris/bars/cut via clip/transform directo). **[HECHO]** **cerebro v2** (`strategy.js`: arco + escenas desde señales del contenido). **[HECHO]** **Ola 5** (+89 modulos).
3. **[PROXIMO · PRODUCCIÓN] perception** — análisis REAL de la página/URL: marca/colores/fotos/mensaje/PÚBLICO/seriedad. Engancha con `backend/` + LLM (ver skill claude-api: Anthropic, modelos al dia). Hoy las señales salen de `content` ya provisto (`core/strategy.js`); perception las llenaria desde una URL real. DECIDIR CON EL USUARIO: que backend, como llamar a Claude, scraping/OG/schema.org. Es la pieza que cierra el ciclo "pego una URL -> sale el video".
4. **scene-buffering -> transiciones REALES de imagen** (dissolve verdadero, glitch-displacement, morph): hoy las transiciones son clip/transform (no mezclan la imagen compuesta). Para esas familias hace falta renderizar A y B a OffscreenCanvas y componer -> requiere un factory de canvas cross-env (browser OffscreenCanvas / Node napi). Solo si se quieren esas familias.
5. **Sin empezar** (blueprint): `composition` (solver de layout — hoy coords hardcodeadas), `photokit` (necesita imágenes reales), `narrative`, `brand-kit`, `post.*` (grade/fx/overlays/**captions animados** — captions pega fuerte pero acompaña a audio/VO).
6. **Más olas** hacia *cientos* por categoría (workflow §5; el contrato de CADA lib —incluidas motion/typekit/transitions— ya esta fijado). Deepen-ear typekit (rutear mas titulos por `TK`).
...
Opción A + transiciones + cerebro v2 + Ola 5 = HECHO (567 modulos, 11 sistemas cableados, engine feature-complete y verificado). **Lo grande que queda: (3) `perception` — el análisis REAL desde URL** (pieza de PRODUCCIÓN: backend + LLM; DECIDIR el approach con el usuario; alimenta `core/strategy.js`). En paralelo, bajo riesgo: más olas hacia cientos (workflow §5), deepen-ear typekit, post.* (grade/fx/captions). Audio al final.
Método: para más MÓDULOS → workflow de agentes (sección 5). Para CABLEADO de uso (perception/composition/post) → orquestador inline (NO en paralelo con un workflow que toque los mismos archivos).
...
- **Ola 4 + cableado completo** (chat jun 2026, 7 fases): color 6→141 (workflow 5 agentes + fix RAIZ `finalize` onAccent-por-contraste + gate). MOTION (12 personalities + `core/motion.js` + 44 escenas usan `M.ease/M.settle` + gate). TYPEKIT (6 efectos + `core/typekit.js`, identidad a reveal=1). MARKKIT/DATAKIT en escenas (datakit al pool de data; garnish icono por rubro). TRANSICIONES (16, sin buffers, clip/transform). CEREBRO v2 (`core/strategy.js`: arco+escenas desde el contenido; 40/40 verificado). Ola 5 (+89: bg/sub/atm/markkit/datakit/scenes). **309 → 567 módulos.** Determinismo + gates + vite build OK en CADA fase. Commits: a34aaa4, 3654e74, 41656c1, 994e9bd, 7b094ab, 90cf96d, 5ff23dd, 6e77c94.
